...
// Function to retrieve new artists without needing to refresh the page
$(document).ready(function () {
    // Function button
    $("#nextArtist").click(function() {
        // AJAX request to get a new artist
        $.ajax({
            url: '/get_new_artist',
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                var artistID = response.artist_id;
                var iframe = document.getElementById('artistFrame');
                iframe.src = 'https://open.spotify.com/embed/artist/' + artistID + '?utm_source=generator&theme=0';
            },
            error: function() {
                alert("Error when retrieving new artist");
            }
        });
    });
});
